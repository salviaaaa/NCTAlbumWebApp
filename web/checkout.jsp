...
<%
    ...
    List<CartBean> selectedItems = cartDAO.getSelectedItems(selectedItemIds);

    double totalPrice = 0;
    for (CartBean item : selectedItems) {
        ...
        <div class="order-summary-card">
            ...
            <% for (CartBean item : selectedItems) { %>
                <div class="d-flex align-items-center mb-2">
                    <img src="data:image/png;base64,<%= item.getImageBase64() %>" alt="<%= item.getAlbumName() %>" class="img-thumbnail mr-2">
                    <div>
                        <div><%= item.getAlbumName() %> - <%= item.getQuantity() %> x Rp <%= item.getAlbumPrice() %></div>
                    </div>
                </div>
            <% } %>
